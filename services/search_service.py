...
Respond in the same language as the user's query."""
    ...
    def __init__(
        ...
        if llm_provider is not None:
            print("LLM provider configured, creating product search tool")
            self.search_tool = create_product_search_tool(product_service)
            print("Product search tool created successfully")
        else:
            print("LLM provider not configured, search will use fallback mode")
    ...
    async def semantic_search(self, query: str) -> SearchResult:
        ...
        if self.llm_provider is None or self.search_tool is None:
            print("LLM not configured, using fallback search")
            return await self._fallback_search(query)

        try:
            print("Attempting LLM search with tool calling")
            result = await self._llm_search(query)
            print(f"LLM search completed successfully, found {len(result.products_found)} products")
            return result
        except Exception as e:
            print(f"LLM search failed: {str(e)}, falling back to direct search")
            return await self._fallback_search(query)

    async def _llm_search(self, query: str) -> SearchResult:
        """Perform search using LLM with tool calling."""
        print("Entering LLM search mode")
        assert self.llm_provider is not None
        assert self.search_tool is not None

        print("Binding tools to LLM model")
        ...
        print("First LLM invocation")
        response = await model_with_tools.ainvoke(messages)

        if hasattr(response, "tool_calls") and response.tool_calls:
            print(f"LLM requested tool calls, executing {len(response.tool_calls)} tool(s)")
            tool_messages = []
            for tool_call in response.tool_calls:
                if tool_call["name"] == "product_search":
                    search_term = tool_call["args"]["search_term"]
                    print(f"Executing product_search tool with term: '{search_term}'")
            ...
            print("Second LLM invocation with tool results")
            messages_with_tools = [
                {"role": "system", "content": self.SYSTEM_PROMPT},
                {"role": "user", "content": query},
                response,
            ...
            print("LLM did not request tool calls, using direct response")
            answer = (
                response.content
    ...
    async def _fallback_search(self, query: str) -> SearchResult:
        ...
        """
        print("Entering fallback search mode")
        search_term = query.strip()

        print(f"Extracting search term from query: '{query}'")
        for word in ["tienen", "hay", "existe", "buscar", "quiero", "stock", "?", "¿"]:
            search_term = search_term.lower().replace(word, "")
        search_term = search_term.strip()

        if not search_term:
            print("No search term extracted from query")
            return SearchResult(
        ...
        print(f"Performing direct search with term: '{search_term}'")
        products = await self.product_service.search_by_name(search_term, limit=10)
        print(f"Fallback search found {len(products)} product(s) for term '{search_term}'")
