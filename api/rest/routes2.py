...
print("Initializing SIFT engine")
sift_engine = get_sift_engine(str(SIFT_STORAGE))
print("SIFT engine initialized successfully")
...
def find_optimal_threshold(cv_image, cv_mask, target_points=1500, tolerance=50):
    ...
    """
    print(f"Finding optimal threshold, target_points={target_points}")
    low = 0.001
    high = 0.2
    best_vis = None
    best_count = -1
    ...
    for iteration in range(8):
        ...
        print(f"Threshold iteration {iteration}: mid={mid:.4f}, count={count}")
        ...
        if abs(count - target_points) <= tolerance:
            print(f"Optimal threshold found: {best_th:.4f} with {best_count} keypoints")
            break
    ...
    print(f"Optimal threshold calculation completed: threshold={best_th:.4f}, keypoints={best_count}")        
...
async def register(
    ...
    """
    print(f"Product registration started for product: {name}")
    try:
        print(f"Reading image from upload - filename: {image.filename}")
        input_bytes = await image.read()
        print(f"Image read successfully, size: {len(input_bytes)} bytes")
        
        print("Processing image to remove background")
        processed_bytes = process_image_bytes(input_bytes)
        print(f"Background removed, processed size: {len(processed_bytes)} bytes")
        ...
        print(f"Image decoded successfully, shape: {cv_image_rgba.shape}")
        
        if cv_image_rgba.shape[2] == 4:
            cv_image = cv2.cvtColor(cv_image_rgba, cv2.COLOR_BGRA2BGR)
            cv_mask = cv_image_rgba[:, :, 3]
            print("Image has alpha channel, mask extracted")
        else:
...
async def preview_keypoints(
    ...
    """
    print("Keypoint preview requested")
    try:
        print(f"Reading image from upload - filename: {image.filename}")
        input_bytes = await image.read()
        ...
        print(f"Keypoint preview completed - threshold: {best_th:.4f}, count: {best_count}")
        ...
        print(f"Exception during keypoint preview: {str(e)}")
        print(traceback.format_exc())
        return JSONResponse(status_code=500, content={'error': str(e)})
...
async def predict(image: UploadFile = File(...)):
    ...
    """
    print("Product prediction requested")
    try:
        print(f"Reading image from upload - filename: {image.filename}")
        ...
        if cv_image is None:
            print("Failed to decode image for prediction")
            return JSONResponse(status_code=400, content={'error': 'Invalid image'})

        print("Identifying product using SIFT engine")
        product_match, matches = sift_engine.identify_product(cv_image)
        
        if product_match:
            print(f"Product identified - name: {product_match['name']}, id: {product_match['id']}, matches: {matches}")
            return {
            ...
            print(f"No product identified, matches: {matches}")
            return {
                'label': 'Unknown',
                'product_id': None,
                'matches': matches,
        ...
        print(f"Exception during product prediction: {str(e)}")
        print(traceback.format_exc())
        return JSONResponse(status_code=500, content={'error': str(e)})
...
def list_versions():
    ...
    """
    print("Listing MLflow model versions")
    try:
        print("Retrieving SIFT_Product_Registry experiment")
        experiment = mlflow.get_experiment_by_name("SIFT_Product_Registry")
        if not experiment:
            print("SIFT_Product_Registry experiment not found")
            return []
        
        print(f"Experiment found, retrieving runs for experiment_id: {experiment.experiment_id}")
        ...
        print(f"Retrieved {len(runs)} model versions")
        versions = []
        for _, run in runs.iterrows():
            ...
            print(f"Version added: {version_info['run_id']}, products: {version_info['product_count']}")
        
        return versions
    except Exception as e:
        print(f"Exception while listing MLflow versions: {str(e)}")
        print(traceback.format_exc())
        return JSONResponse(status_code=500, content={'error': str(e)})
...
def restore_version(request: RestoreRequest):
    ...
    """
    print(f"Model version restoration requested - run_id: {request.run_id}")
    run_id = request.run_id
    if not run_id:
        print("No run_id provided for restoration")
    ...
    try:
        ...
        print(f"Copying downloaded model to {SIFT_STORAGE}")
        shutil.copy(downloaded_path, SIFT_STORAGE)
        
        print("Reloading SIFT engine database")
        sift_engine.load_database()
        
        db_size = len(sift_engine.database)
        print(f"Model version restored successfully - run_id: {run_id}, database size: {db_size}")
        return {'message': f'Restored version {run_id}', 'count': db_size}
    except Exception as e:
        print(f"Exception during model restoration: {str(e)}")
        print(traceback.format_exc())
        return JSONResponse(status_code=500, content={'error': str(e)})
...
async def analyze_video(video: UploadFile = File(...)):
    ...
    """
    print(f"Video analysis requested - filename: {video.filename}")
    temp_file_path = f"temp_{video.filename}"
    try:
        print(f"Writing uploaded video to temporary file: {temp_file_path}")
        with open(temp_file_path, "wb") as buffer:
            shutil.copyfileobj(video.file, buffer)
        
        print("Processing video")
        result_text = process_video(temp_file_path, sift_engine)
        print(f"Video analysis completed successfully - filename: {video.filename}")
        
        return {"script": result_text}
    except Exception as e:
        print(f"Exception during video analysis: {str(e)}")
        print(traceback.format_exc())
        return JSONResponse(status_code=500, content={'error': str(e)})
    finally:
...
async def chat(request: ChatRequest):
    ...
    """
    print(f"Chat message received - length: {len(request.message)} characters")
    try:
        message = request.message
        image = request.image
        
        print("Annotating message with product SKU-IDs")
        ...
        print(f"Exception during chat processing: {str(e)}")
        print(traceback.format_exc())
        return JSONResponse(status_code=500, content={'error': str(e)})
